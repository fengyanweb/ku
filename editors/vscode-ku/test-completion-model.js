...
assert.deepStrictEqual(model.memberCompletionLabels("string", "ch"), ["chars"]);
assert.deepStrictEqual(model.memberCompletionLabels("text", "ch"), ["chars"]);
assert.deepStrictEqual(model.memberCompletionLabels("string", "by"), ["byte_len"]);
assert.deepStrictEqual(model.memberCompletionLabels("text", "by"), ["byte_len"]);
assert.deepStrictEqual(model.stdRootModules, ["fs", "http", "string", "array", "object", "json", "config", "time", "task", "pg", "redis", "mysql"]);
assert.deepStrictEqual(model.memberCompletionLabels("pg"), ["client"]);
assert.deepStrictEqual(model.memberCompletionLabels("redis"), ["client"]);
assert.deepStrictEqual(model.memberCompletionLabels("mysql"), ["client"]);
assert.ok(!model.stdFunctions.includes("pg.connect"));
assert.ok(!model.stdFunctions.includes("pg.pool"));
assert.ok(!model.stdFunctions.includes("pg.query_params"));
assert.ok(!model.stdFunctions.includes("redis.connect"));
assert.ok(!model.stdFunctions.includes("redis.auth"));
assert.ok(!model.stdFunctions.includes("redis.get_required"));
assert.ok(!model.stdFunctions.includes("mysql.connect"));
assert.ok(!model.stdFunctions.includes("mysql.query_params"));
...
assert(model.globalCompletionLabels().includes("string.chars"));
assert(model.globalCompletionLabels().includes("string.byte_len"));
