...
- 可导入的 `ParseTypeWindow` 不依赖 `ReadTypeAt` 维持安全前置条件：它会在任何 token 索引前拒绝空窗口和超过 512 个 token 的窗口，再用至多 512 步确认只有最后一个 token 是 non-type boundary；缺失或提前出现 boundary 都返回 `bootstrap.parser.stage3/invalid_token_stream`，验证步数也进入工作量计数。
