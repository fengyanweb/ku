...
- Stage 3 是刻意收窄的生产语法子集：语句边界沿用生产 parser 的 token grammar，已覆盖 Ku 源码通常使用的换行/空白分隔方式；已有可选分号仍能解析，但不是新增或推荐的第二种写法。函数类型、泛型、async、enum 和控制流尚未进入该切片，`string`/`nil` 也不会作为类型别名接受；连续 `T!!` 也不会成为第二种 Result 写法，嵌套 Result 通过结构本身表达，例如 `[T!]!`。完整模块限制为 131072 bytes、32768 个 Unicode 字符、512 个 token（含 EOF）、4096 个 arena node、8192 条 arena edge 和 16384 个 parser work step；byte/char/token 在任何全表 clone 前检查，node/edge 在 append 前检查。另有每个 expression boundary 4096 步、token-byte 到字符位置映射 36864 步、`string.slice` 聚合 131072 个源码字符访问、type nesting 32 层和每个 union 层级 64 个 arm；每模块 64 个 import、每个 named import 64 个名字、每个函数 32 个参数和 128 条语句、每个模块 64 个函数。module、struct 和 struct field 只受这些总 token/work/arena 上限约束，不另造独立数量规则。import inspection window 另有 266 token 硬上限，可完整覆盖第 65 个全 alias 项并让 64-name 语法上限先行生效；type inspection window 不超过 512 token。所有循环都受 EOF、类型深度、union 宽度、token/item 数量或工作预算约束，并在每轮推进显式游标；超限和不支持的形式稳定返回 `bootstrap.parser.stage3` 诊断，不会猜测第二套语法。
- Stage 2/3 的 Rust AST canonical 差分、空输入、module/import/struct/function 交错顺序、Unicode/CRLF byte span、malformed module/import/struct/签名错误、深度/数量边界，以及解释器/native 源码删除后运行均有独立测试。另有不经过运行时 Rust projector 生成 expected 的 checked-in goldens，固定 Stage 2 二元/调用树、Stage 3 module/struct/typed function/union 树和 parser Diagnostic 的 severity/domain/code/source/message/span；escape-sensitive golden 还覆盖字段分隔符、反斜杠、CR/LF、tab 与 Unicode，避免 Rust projector 与 Ku parser 同步漂移后自证。native allocation gate 一组使用 32/64 个单字段 struct（分别为含 EOF 的 225/449 token、97/193 个 node、96/192 条 edge），另一组使用单个含 16/32 个 custom union arm 的 struct（38/70 token、20/36 个 node、19/35 条 edge），分别隔离声明窗口和复杂 type-plan 展平；第三组使用缺失右花括号的 32/64-field complex struct（含 EOF 为 164/324 token），确认字段和类型节点已分配后仍通过结构化错误释放完整中间 arena。每组执行 8 轮，检查输入 fingerprint 不变、每轮 drop 后 zero-live。声明数量 gate 对 allocation call/requested bytes/peak live bytes 依次使用 `2.5 * small + 64/64 KiB/32 KiB` 上界；复杂 union gate 使用 `2.25 * small + 32/8 KiB/4 KiB`；malformed cleanup gate 使用 `2.25 * small + 64/16 KiB/8 KiB`。前一门槛能拒绝“每解析一个 struct 就深拷贝完整 token 表”的增长回归，复杂类型门槛能拒绝字段窗口或 `AddParsedType` 每轮重复读取 growing owned array 的回归；它们仍是有限规模有界亚二次证据，不是严格渐近线性或生产吞吐证明。Stage 3 的 import 这里只生成声明 AST，不负责依赖图加载；那部分仍由 Rust bootstrap compiler 承担。C artifact 仍是无 C compiler 环境下的硬门槛；只有链接器实际可用时才执行 native 二进制。
...
1. Ku parser：已有表达式 parser 和含 module/import/struct、基础函数签名的最小函数/语句模块切片，但函数类型、泛型、enum、控制流与其余表达式仍未迁移；不能称为完整 Ku parser。当前 Ku `Error` 仍只有 `domain/code/message` 三个字段，不是任意 typed payload 通道；Stage 2/3 通过 `ParseContext` 在诊断序列化前完成 span/domain 重定位，而不是反向解析 `message`。
...
3. 在现有 Stage 2/3 module/import/struct/函数签名差分门槛上逐块加入 enum 和控制流；每个新增节点先固定 canonical AST/diagnostic，再扩展语法。
