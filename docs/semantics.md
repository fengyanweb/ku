...
Parser 与 Checker 对可执行语句体的嵌套独立限制为 32 层；顶层函数、括号和叶子语句
不计入该语句体层数。if/else-if、循环、try/catch/finally、局部函数和各种函数值体
不能通过互相嵌套重置计数。表达式和类型仍有各自既有的深度限制。
Checker 在克隆函数体和捕获扫描前迭代预检初始 AST；超限明确报错，不扩大线程栈。

循环所有权推演另有每个 Checker 共享的 100,000 工作单位：每次推演轮次和推演中的
语句检查入口访问都会计费，嵌套或兄弟循环不重置预算。耗尽报告
`maximum check work exceeded; loop ownership analysis budget exhausted`，
被忽略的推演错误不能使不完整分析成为成功结果。没有外部绑定时只省去空推演，
正式类型和所有权检查仍执行；这不是限制用户程序运行时的循环次数。

这些是具体前端边界，不是完整的编译器内存、总 CPU 或任意 raw AST 安全证明。
纯表达式/类型/模式的 raw AST clone/drop、模板重新解析的累计预算和分配失败
仍须分别验证；不能据此宣称所有输入路径已经闭环。
