...
| TLS client | runtime 地基已固定，标准库集成未完成 | 解释器的 `http.get/post/request` 和 registry HTTPS 使用 rustls、TLS 1.2/1.3、WebPKI 根与主机名验证；`ku-registry` 有专用 rustls TLS listener；libpq 可使用 PostgreSQL TLS。新的 socket-free native TLS ABI 使用 rustls/ring，只接受内置 WebPKI 根或完全替换它的显式 PEM CA；证书与主机名验证不可关闭，也没有明文回退；runtime crate 已在三系统 CI 构建和测试 | native TLS ABI 尚未接入 generated C 的 `net.client(config)`、HTTP 或 Redis，也尚无最终消费者链接门槛；解释器 stream parity 未完成；它不读取系统根，尚无 TLS server/mTLS，不能称为通用 native HTTPS |
| PostgreSQL | 统一 client 实验实现 | 只公开 `pg.client(config)` 与 receiver API；内部有界池、服务端参数绑定、单行增量聚合、NULL/空串区分、严格 UTF-8/结果上限；发送后模糊失败为 `execution_unknown`，成功终态后结果失败为 `execution_completed_without_result`；明确 session-control 与执行后非 IDLE 返回 `session_state_unsupported`，正常归还前在原预算内 `DISCARD ALL`；conninfo 内部副本销毁前清零；三系统精确 libpq 链接/启动和 workspace CI 已通过 | Windows loopback PostgreSQL 新 API 已跑通，Linux/macOS 实库查询仍待验；SQL 文本预检不能证明任意过程/函数没有状态或外部副作用；同步 DNS/libpq/SSL 内部调用不能硬抢占；不支持 transaction/COPY/exclusive；每次安全 reset 多一次往返；结果 cap 不是进程峰值硬上限；自动静态链接 libpq 不支持 |
| owned bytes / plain TCP | native 三平台基础实现 | `bytes.from_str/from_array` 是唯一构造入口，`len/get/to_str` 是唯一读取入口；深 clone、move/drop、array/Result/closure 路径使用同一 ABI；`net.client(config)` 只公开 `read/write/close`，单连接 gate、connect/read/write 总 deadline、单次读取与 16 MiB 配置硬上限均有 loopback/生成 C 测试，三系统 workspace CI 已跑绿 | 当前仅 native C，`ku run` 尚不支持；`read(n)` 是一次最多 n bytes 的 `recv`，不是 read-exact/message API；明文 TCP，不含 TLS；同步 `getaddrinfo` 不能硬取消；CI 通过不等于生产弱网 soak |
...
- 三个数据库 client 都由 Ku move-only/checker 管理：`close()` 消耗并清空唯一 owner，只读 handler 不能关闭捕获的外层 client。生成的数据库 helper 是 translation-unit 内部 `static` 实现而非第三方 C ABI。内部 `finalizing` 只仲裁仍存活 allocation 上已进入的 close/release 路径，确保最多一个销毁者；它不支持重复 close、close 返回后使用裸指针，或 operation 尚未登记时与 consuming close 同时开始。
