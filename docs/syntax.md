...
当前 native C 子集要求 import 展开后的函数全部是顶层非泛型 async 函数，入口为无参数 `async fn main(): null!`；参数限 `int/bool/null/str` 或对应单层 Result，返回显式 primitive `T!`。函数体支持局部绑定、已有 int/bool/null 局部的普通赋值、源码 `if` / `else` 及分支词法作用域、已知 async 调用、Task move、Await、ok、?、primitive print/println、显式 return 和字符串常量 fail；Copy 表达式支持 checked int 的一元 `-`、`+ - * / %` 和 `== != < <= > >=`，bool 的 `!`、`== !=` 及 `&& ||` 短路。普通二元式左值先求值并保存，右侧 Await 不会丢失该快照；未选中的逻辑右侧或 If 分支不执行其调用、move 或 Await，但仍静态检查。溢出/除零保持不可恢复 runtime failure，不因 async 返回 `T!` 就改成普通 Err。新字符串表达式限静态字面量。循环/递归、Owned/Task 重赋值、复合赋值与自增/自减、跨词法作用域 Task move、try/catch/finally、闭包/函数值、同步用户函数调用、借用 async 参数、Task 参数/返回/容器/clone、未绑定 Task 临时、float/混合类型算术、str/null/Result/Task 比较和动态堆表达式仍拒绝。示例与完整边界见 [当前 native C 源码子集](concurrency.md#当前-native-c-源码子集)。`ku ir`、`--emit-ir` 和 LLVM 不通过这条 Task lowering 路径。
