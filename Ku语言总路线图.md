...
> 说明：这是一份早期长期路线图，保留用于记录大方向和历史上下文；它不逐条代表当前 0.0.12+ 的真实实现状态。当前能力边界以 README、docs/syntax.md、版本记录和 roadmap-decisions.md 为准。
...
```
Ku 要想稳定、快、省资源、语法简单，就不能什么都做。

- 语法体验像 Go：简单、直接、适合服务端。
- 语义规则像 Rust：默认严格、错误明确、少隐式。
- 运行时并发像 Go：HTTP 和 async 默认并发，用户不管理线程池。
- 资源控制像 Zig/Rust：默认有上限，不无限吃内存。
- 数据结构逐步 native 化，标准库强约束资源，错误保持结构化。
- 后端尚未支持的能力明确拒绝，不生成似是而非的结果。
